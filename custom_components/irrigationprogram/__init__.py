...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def _async_finish_setup(_event=None):
        ...
        for zone in config.get(ATTR_ZONES,[]):
            ...
            if state is None or state.state in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                msg_parts.append(
                    f"ERROR {z.zone} has not initialised before the Irrigation Program, "
                    "this could be a slow registering device, try increasing the "
                    "'Wait time from devices that load slowly on startup' setting "
            ...
            if z.adjustment:
                ...
                if state is None or state.state in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                    msg_parts.append(
                        f"Warning, {z.adjustment} has not initialised before "
                        "irrigation program, check your configuration"
                    )
            if z.rain_sensor:
                state = hass.states.get(z.rain_sensor)
                if state is None or state.state in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                    msg_parts.append(
                        f"Warning, {z.rain_sensor} has not initialised before "
                        "irrigation program, check your configuration"
                    )
